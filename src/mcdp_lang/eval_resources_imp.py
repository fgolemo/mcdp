...
from mcdp_posets.rcomp_units import RCompUnitsPower
import numpy as np
...
def eval_rvalue(rvalue, context):
    ...
    try:
        if isinstance(rvalue, CDP.Divide):
            from mcdp_lang.eval_math import eval_divide_as_rvalue
        ...
        if isinstance(rvalue, CDP.MultN):
            from mcdp_lang.eval_math import eval_MultN_as_rvalue
            return eval_MultN_as_rvalue(rvalue, context)

        if isinstance(rvalue, CDP.PlusN):
            from mcdp_lang.eval_math import eval_PlusN_as_rvalue
